...
const LOGIN_BLOCK_MS = 15 * 60 * 1000; // 15 menit
const loginAttempts = new Map(); // email -> { count, lastAt, blockedUntil }

function loginRateLimitError(email) {
  const now = Date.now();
  ...
  if (now - rec.lastAt > LOGIN_WINDOW_MS) {
...
function recordLoginFailure(email) {
  const now = Date.now();
  const rec = loginAttempts.get(email) ?? { count: 0, lastAt: now };
  rec.count += 1;
  rec.lastAt = now;
...
export async function loginAdmin(formData) {
  ...
  const rateLimitError = loginRateLimitError(email);
  if (rateLimitError) {
