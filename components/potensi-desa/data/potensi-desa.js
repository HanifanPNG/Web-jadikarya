export const potensiDesaData = [
  {
    id: "agroforest",
    title: "Sektor Agroforestik",
    subtitle: "Potensi Pertanian & Perhutanan Sosial",
    heroImage: "/assets/agroforestik/agroforestik3.webp",
    description:
      "Didukung tanah yang subur, Desa Jadikarya menghasilkan berbagai komoditas unggulan seperti jagung, kapulaga, durian, alpukat, manggis, dan lada yang menjadi penopang perekonomian masyarakat.",
    detailDescription:
      "Melalui sistem agroforestri, masyarakat mengombinasikan tanaman kehutanan dengan tanaman produktif sehingga hutan tetap lestari sekaligus memberikan manfaat ekonomi. Kelompok Tani Hutan (KTH) Lebak Jero mengelola sekitar 65 hektare lahan perhutanan sosial, termasuk area budidaya jagung yang didukung program swasembada pangan dan penyuluhan rutin kepada petani desa.",
    commodities: ["Jagung", "Kecombrang", "Durian", "Alpukat", "Manggis", "Tomat", "Kelapa"],
    highlights: [
      { label: "Luas Perhutanan Sosial", value: "65 Hektare" },
      { label: "Program", value: "Ketahanan Pangan Nasional" },
    ],
    color: "from-emerald-600 to-green-700",
  },
  {
    id: "gudang-buah",
    title: "Gudang Buah (Sumber Rezeki)",
    subtitle: "Sortir & Distribusi Buah",
    heroImage: "/assets/gudangBuah/gudang-buah1.webp",
    description:
      "Gudang buah durian dan manggis merupakan pusat sortir dalam negeri maupun luar negeri yang menjadi sumber rezeki utama masyarakat Desa Jadikarya.",
    detailDescription:
      "Melalui gudang buah ini, hasil panen durian dan manggis warga disortir secara higienis sebelum didistribusikan ke pasar dalam negeri maupun diekspor. Keberadaannya memperpendek rantai distribusi sehingga harga komoditas lebih menguntungkan bagi petani dan membuka lapangan kerja bagi warga setempat.",
    commodities: ["Durian", "Manggis", "Gudang Sortir", "Distribusi & Ekspor"],
    highlights: [
      { label: "Fungsi", value: "Gudang Sortir Nasional" },
      { label: "Target", value: "Dalam Negeri & Luar Negeri" },
    ],
    color: "from-amber-600 to-orange-700",
  },
  {
    id: "persawahan",
    title: "Sektor Persawahan & Perkebunan",
    subtitle: "Lanskap Alam & Potensi Wisata",
    heroImage: "/assets/persawahan/sawah.webp",
    description:
      "Keindahan sungai alam dan perbukitan hijau Langkaplancar potensial dikembangkan menjadi destinasi wisata arung jeram dan desa wisata berkelanjutan.",
    detailDescription:
      "Hamparan sawah bersama sungai alami dan perbukitan hijau menjadikan wilayah ini tidak hanya subur bagi sektor pertanian, tetapi juga sangat menjanjikan untuk dikembangkan sebagai destinasi wisata alam, arung jeram, dan desa wisata berkelanjutan yang mendukung ekonomi warga secara mandiri.",
    commodities: ["Persawahan", "Sungai Alam", "Arung Jeram", "Desa Wisata"],
    highlights: [
      { label: "Potensi", value: "Wisata Arung Jeram" },
      { label: "Jenis", value: "Desa Wisata Berkelanjutan" },
    ],
    color: "from-sky-600 to-blue-800",
  },
  {
    id: "umkm",
    title: "UMKM Pusat Unggulan Jadikarya",
    subtitle: "Peternakan & Perikanan",
    heroImage: "/assets/umkm/kios.webp",
    description:
      "Pemeliharaan ternak sapi, kambing, dan budidaya ikan air tawar yang dikelola oleh kelompok tani dan pemuda desa untuk ketahanan pangan lokal.",
    detailDescription:
      "UMKM menjadi penggerak ekonomi desa melalui pemeliharaan ternak sapi, kambing, dan budidaya ikan air tawar yang dikelola bersama kelompok tani dan pemuda desa. Upaya ini memperkuat ketahanan pangan lokal sekaligus membuka peluang pendapatan baru bagi masyarakat hingga meningkatkan kesejahteraan warga.",
    commodities: ["Kripik Singkong", "Gula aren", "Gula Kelapa", "Pengolah Kayu", "Peternak domba"],
    highlights: [
      { label: "Kelola", value: "Kelompok Tani & Pemuda Desa" },
      { label: "Fokus", value: "Ketahanan Pangan Lokal" },
    ],
    color: "from-rose-700 to-pink-800",
  },
  {
    id: "pendidikan",
    title: "Sektor Pendidikan",
    subtitle: "Pengembangan SDM Masyarakat",
    heroImage: "/assets/pendidikan/sd_2_jadikarya.webp",
    description:
      "Pengembangan sumber daya manusia melalui peningkatan akses dan mutu pendidikan di Desa Jadikarya.",
    detailDescription:
      "Pendidikan menjadi fondasi pembangunan berkelanjutan di Desa Jadikarya. Melalui dukungan kelembagaan desa, pemerintah desa mendorong peningkatan akses dan mutu pendidikan serta pelatihan keterampilan bagi warga agar dapat berperan aktif dalam pembangunan ekonomi dan sosial desa.",
    commodities: ["PAUD & Sekolah", "Pelatihan", "Beasiswa Desa"],
    highlights: [
      { label: "Fokus", value: "SDM & Keterampilan Warga" },
      { label: "Dukungan", value: "Pemerintah & Kelembagaan Desa" },
    ],
    color: "from-indigo-600 to-violet-700",
  },
  {
    id: "kesehatan",
    title: "Sektor Kesehatan",
    subtitle: "Layanan & Ketahanan Kesehatan",
    heroImage: "/assets/kesehatan/posyandu_1.webp",
    description:
      "Peningkatan layanan kesehatan dan kesadaran hidup sehat masyarakat yang didukung oleh fasilitas posyandu dan tenaga kesehatan desa.",
    detailDescription:
      "Kesehatan masyarakat menjadi prioritas pembangunan Desa Jadikarya. Fasilitas posyandu dan layanan kesehatan yang didorong bersama tenaga kesehatan desa memberikan akses layanan yang baik serta mendukung terwujudnya masyarakat yang sehat dan produktif.",
    commodities: ["Posyandu", "Layanan Kesehatan", "Penyuluhan Sehat"],
    highlights: [
      { label: "Fokus", value: "Kesadaran Hidup Sehat" },
      { label: "Layanan", value: "Posyandu & Tenaga Kesehatan" },
    ],
    color: "from-teal-600 to-cyan-700",
  },
  {
    id: "layanan-publik",
    title: "Pelayanan Masyarakat",
    subtitle: "Layanan Administrasi & Publik",
    heroImage: "/assets/pelayanan/kades.webp",
    description:
      "Pelayanan yang mudah, cepat, dan transparan untuk seluruh warga melalui sistem pelayanan terpadu pemerintahan desa.",
    detailDescription:
      "Pemerintah Desa Jadikarya berkomitmen memberikan pelayanan administratif yang mudah, cepat, dan transparan kepada masyarakat melalui sistem pelayanan terpadu. Layanan tersebut mencakup administrasi kependudukan, surat-menyurat, serta berbagai kebutuhan publik yang mendukung kesejahteraan warga secara mandiri dan inklusif.",
    commodities: ["Administrasi", "Surat Menyurat", "Layanan Terpadu"],
    highlights: [
      { label: "Sistem", value: "Pelayanan Terpadu Desa" },
      { label: "Komitmen", value: "Cepat, Mudah, Transparan" },
    ],
    color: "from-slate-600 to-zinc-700",
  },
];